public class Car {
    ...
    private static final int NEXT_ROAD_INDEX = 1;
    ...
    public void move() {
        ...
        if (!this.currentRoad.getLightsOnRoad().isEmpty() //if the traffic light is red
            ...
            //when current road is finished, connect to next road.
            if (this.currentRoad.getLength() == this.getPosition() && !this.currentRoad.getConnectedRoads().isEmpty()) {
                ...
                //the car is still moving on the road
    ...
    public void printCarStatus() {
        System.out.printf("%s --> speed: %dm/s, on %s, at position:%s%n", this.getId(), this.getSpeed(), this.getCurrentRoad().
